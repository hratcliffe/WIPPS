 ...
 * Overall setup is via command line arguments, detailed by the ./main -h option. These specify the domain, fields, and working directory/file prefix. On multiple processors, each handles several space blocks sequentially. Physical parameters are extracted from the [prefix]deck.status file generated by the code. The plasma is defined in class plasma. Number of components is fixed, and the densities, charges and masses are set from [prefix]plasma.conf
