...
*/
  ...
  if(file.is_open()){
    ...
    file<<"TAN_MIN "<<TAN_MIN<<'\n';
    file<<"TAN_MAX "<<TAN_MAX<<'\n';
...
*/
  ...
  if(!minor){
    if(major_in > major_v) return 1;
    else if(major_in == major_v) return 0;
    else return -1;
  }else{
    if(major_in > major_v) return 1;
    else if(major_in < major_v) return -1;
    else{
      if(minor_in > minor_v) return 1;
      else if(minor_in == minor_v) return 0;
      else return -1;
    }
  }
}
